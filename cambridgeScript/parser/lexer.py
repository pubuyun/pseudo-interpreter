...
def parse_tokens(code: str) -> list[Token]:
    ...
    for match in re.finditer(_TOKEN_REGEX, code, re.M):
        ...
        if token_type == "SYMBOL" and token_value == "-":
            if (
                last_token is None  # first token
                or isinstance(last_token, SymbolToken)  # after symbol
                or isinstance(last_token, KeywordToken)  # after keyword
                or isinstance(last_token, EOFToken)
            ):  # EOF
                # try to match next token
                match_next = re.match(_TOKEN_REGEX, code[match.end() :])
                if match_next and match_next.lastgroup == "LITERAL":
                    literal_value = code[
                        match.end() : match.end() + len(match_next.group())
                    ]
                    token_value = f"-{literal_value}"
                    token_type = "LITERAL"
                    match = re.match(
                        _TOKEN_REGEX,
                        code[match.start() : match.start() + len(token_value)],
                    )
                else:
                    raise InvalidTokenError(
